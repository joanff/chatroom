...
res = post('http://localhost:5000/create_room', json={'room':{'name':'testroom2'}}).json()
print(res)


# Adding messages to rooms
# add a message content='test content two', room_id=2
res = post('http://localhost:5000/add_msg', json={'message':{'content':'test content two', 'room_id':2}}).json()
print(res)

# "liking" a message
# add a like user_id=1, room_id=1
res = post('http://localhost:5000/add_like', json={'user_id':1, 'message_id':1}).json()
print(res)
